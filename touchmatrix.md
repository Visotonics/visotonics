...
| `/lab/*` | `app/lab/**/page.tsx` | **experimental — see note below** | 9 routes (see the lab note directly below) | varies |

**Lab routes are experimental prototypes: never linked from nav/footer, never in `sitemap.xml`, and every one sets `robots: { index: false, follow: false }`.** They exist for reviewing in-progress work (mostly the three.js "vision scene" prototypes) in isolation from the live pages. Current routes:

| Route | File | Purpose |
|---|---|---|
| `/lab/home` | `app/lab/home/page.tsx` | Full clone of the homepage wired to the four `_vision/lazy` hero-card scenes, for reviewing the live cards in the real page layout |
| `/lab/home-layouts` | `app/lab/home-layouts/page.tsx` | Four static-placeholder hero-band layout iterations, stacked for side-by-side comparison (deliberately uses flat panels, not live WebGL — see the file's own comment on the 16-context browser cap) |
| `/lab/hero-cards` | `app/lab/hero-cards/page.tsx` + `app/lab/hero-cards/{index,card-scene,subjects,detect,ground,skins}.ts(x)` | Reproduces the homepage's desktop hero-card band geometry so the four card scenes (Yard/Warehouse/Factory/Data) can be reviewed side by side at real on-page size, independent of the homepage |
| `/lab/lead-card` | `app/lab/lead-card/page.tsx` + `scene.tsx` | Standalone stage for the lead-card scene at its real light-surface proportions |
| `/lab/container-vision` | `app/lab/container-vision/page.tsx` + `{scene,container,materials,palette,hud}.ts(x)` | Standalone stage for the Container Vision flagship scene, at the schematic's 1600×680 aspect ratio |
| `/lab/tank-vision` | `app/lab/tank-vision/page.tsx` + `scene.tsx` | Standalone stage for Tank Vision — split out because reviewing it via `/lab/viso-yard` means loading the whole 20,000px page with two other live WebGL scenes on it |
| `/lab/gate-vision` | `app/lab/gate-vision/page.tsx` + `{scene,gate,materials}.ts` | Standalone stage for the Gate Vision flagship scene |
| `/lab/yard-vision` | `app/lab/yard-vision/page.tsx` + `{scene,yard}.ts(x)` | Standalone stage for Yard Vision, section 04's flagship. Unlike the Tank/Container lab pages this one has a DARK page background, because the section is dark and the scene's container hexes are chosen against it. Accepts `?phase=0..1` to pin the loop position for review |
| `/lab/viso-yard` | `app/lab/viso-yard/page.tsx` + local `rail`/`sections`/`convert`/`_shared`/`_media`(implied) | Full clone of `/platform/viso-yard` used as an iteration ground for the flagship scenes in their real page context; imports `SectionSecure`/`SectionWork` from its own `sections.tsx`, not `../viso-warehouse/sections` as the production route does |

Shared lab infrastructure lives in `app/lab/_vision/` (imported by the lab routes above, and by `app/lab/home/page.tsx` — the only non-`lab/` importer found): `lazy.tsx` (`ContainerVisionScene`, `TankVisionScene`, `GateVisionScene`, `YardVisionScene`, `LeadCardScene`, `YardCard`, `WarehouseCard`, `FactoryCard`, `DataCard` — note `YardVisionScene` is section 04's aerial flagship and `YardCard` is the small homepage hero tile; different scenes, similar names — each a `WhenNear`-gated dynamic import, see PERFORMANCE.md/DECISIONS.md for the idle-build behaviour), `mount.ts` (`mountWhenVisible`), `studio.ts` (`createStudio`, `StudioOpts`/`Studio` — the shared renderer/PMREM/composer/lights rig; `container-vision/scene.tsx` is the one flagship that still doesn't use it, deliberately, per DECISIONS.md), `metal.ts` (`makeMetal`, `tintMetal`, `warmMetalCache`, `CANONICAL_BRUSHED`, `metalBox`, `MetalKind`/`MetalOpts`/`Metal`), `camera.ts` (`makeCamPath`, `blendPose`, `placeCamera`, `CamKey`/`CamPose`/`Opening`, easing helpers), `overlay.ts` (`createCallout`, `placeCallout`, `createReadout`, `makeProjector`, `Lane`/`CalloutSpec`/`Callout`/`Readout`; `CalloutSpec.onDark` switches the leader line to light ink for scenes on the dark canvas), `palette.ts` (`PALETTE`, `sans`).
...
- `app/lab/_vision/*` → every `/lab/*` route plus `app/lab/home/page.tsx`; no production (non-lab) route imports from `app/lab/**` (verified by grep) — the lab tree is currently a dead end for blast-radius purposes, but treat that as a standing invariant to re-check, not a permanent guarantee.
- `app/lab/viso-yard/` and `app/dev/viso-yard-light/` each carry their **own** forked copies of `_shared`/`rail`/`sections`/`convert` rather than importing the production `app/platform/viso-yard/` modules — edits to the production Yard route do not propagate to either fork, and vice versa.
- `app/lab/yard-vision/yard.ts` → imports container skins from `app/lab/hero-cards/skins.ts`, so the lab hero-card module is now a dependency of a flagship scene, not just of the card row. Changing those skin generators affects Yard Vision's 55 containers as well as the homepage cards.
- `app/lab/gate-vision/materials.ts` and `app/lab/yard-vision/yard.ts` both export a `warm*Textures()` that `_vision/lazy.tsx` calls during idle. A new scene that generates textures needs the same pair (module-level cache + exported warm) or its cost lands on the visitor's scroll — see PERFORMANCE.md.
