...
// LIGHT-THEME VARIANT: band-corner registration crosses were dark ink on the
// light Tank/Platform bands; those bands are now dark (inverted), so this is
// light ink instead.
const CROSS_INK = "rgba(244,245,247,0.45)";
...
function CheckpointBox({ detection = false }: { detection?: boolean }) {
  const b = "1px solid rgba(244,245,247,0.8)"; // light-theme variant: light ink on the now-dark band
  return (
          ...
          {STATIONS.map((label, i) => (
            <div key={label} style={{ display: "flex", alignItems: "center", gap: 16, paddingLeft: 4 }}>
              <span style={{ position: "relative", flex: "0 0 auto", display: "flex", flexDirection: "column", alignItems: "center" }}>
                <span aria-hidden="true" style={{ width: 10, height: 10, border: "1px solid rgba(244,245,247,0.8)", background: label === "CRANE OFF" ? "rgba(237,81,12,0.14)" : "transparent", borderColor: label === "CRANE OFF" ? SIGNAL : "rgba(244,245,247,0.8)" }} />
